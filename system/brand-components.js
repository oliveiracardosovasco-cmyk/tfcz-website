...
(function (root) {
  ...
  /* Die Datei traegt die haeufigsten Icons als Markup bei sich — sie muss auch
     ohne Netz und ohne Studio zeichnen koennen (Brand Guide, Vorschauen).
     Alles Uebrige kommt aus window.TFCZ_ICON_EXTRA: dort legt die Icon-Bibliothek
     (system/icon-picker.js) nachgeladene Lucide-Icons ab. So steht dem Studio die
     ganze Lucide-Sammlung offen, ohne dass 1750 Pfade hier hineinwachsen. */
  function ikonInnen(name) {
    if (LU[name]) return LU[name];
    var EX = root && root.TFCZ_ICON_EXTRA;
    return (EX && EX[name]) || '';
  }
  function icon(name, sz) {
    var inner = ikonInnen(name);
    if (!inner) return '';
  ...
  var COMPONENTS = {
    ...
    icon: {
      ...
      felder: [
        { k: 'icon', label: 'Icon', typ: 'icon' },
        { k: 'farbe', label: 'Farbe', typ: 'select', opts: Object.keys(FARBEN) },
        { k: 'size', label: 'Grösse (px)', typ: 'zahl' }
      ],
      defaults: function () { return { icon: 'trophy', farbe: 'gold-lt', size: 64 }; },
      /* Welche Icons diese Komponente braucht — das Studio laedt sie vor dem Zeichnen nach. */
      iconsAus: function (p) { return [p.icon]; },
      render: function (p, k) {
        k = k || 1;
    ...
    liste: {
      ...
      iconsAus: function (p) {
        return String(p.zeilen || '').split('\n').filter(Boolean).map(function (l) { return (l.split('|')[0] || '').trim(); });
      },
      render: function (p, k) {
  ...
  root.TFCZ_COMPONENTS = { list: COMPONENTS, ziele: ZIELE, btn: BTN, icon: icon, hat: function (n) { return !!LU[n]; }, textstyles: TEXTSTYLES, farben: FARBEN, textCss: textCss };
